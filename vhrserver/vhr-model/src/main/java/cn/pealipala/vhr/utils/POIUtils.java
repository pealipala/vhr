...
public class POIUtils {
    public static ResponseEntity<byte[]> employee2Excel(List<Employee> list) {
        ...
        //文档管理员
        information.setManager("pealipala");
        //文档公司信息
        information.setCompany("www.pealipala.cn");
        //4.获取文档摘要信息
        SummaryInformation summaryInformation = workbook.getSummaryInformation();
        //文档标题
        summaryInformation.setTitle("员工信息表");
        //文档作者
        summaryInformation.setAuthor("pealipala");
        //文档备注
        summaryInformation.setComments("本文档由pealipala提供");
                            ...
                            case STRING:
                                ...
                                switch (k) {
                                    ...
                                    case 6:
                                        ...
                                        break;
                                    case 7:
                                        for(int n=0;n<allNations.size();n++){
                                            if (cellValue.equals(allNations.get(n).getName())){
                                                employee.setNationid(allNations.get(n).getId());
                                                break;
                                            }
                                        }
...
//                                        break;
                                    case 8:
                                        ...
                                        break;
                                    case 9:
                                        for(int ap=0;ap<allPoliticsstatus.size();ap++){
                                            if (cellValue.equals(allPoliticsstatus.get(ap).getName())){
                                                employee.setPoliticid(allPoliticsstatus.get(ap).getId());
                                                break;
                                            }
                                        }
                                        break;
                                    ...
                                    case 12:
                                        for(int d=0;d<allDepartments.size();d++){
                                            if (cellValue.equals(allDepartments.get(d).getName())){
                                                employee.setDepartmentid(allDepartments.get(d).getId());
                                                break;
                                            }
                                        }
                                    case 13:
                                        for(int l=0;l<allJobLevels.size();l++){
                                            if (cellValue.equals(allJobLevels.get(l).getName())){
                                                employee.setJoblevelid(allJobLevels.get(l).getId());
                                                break;
                                            }
                                        }
                                    case 14:
                                        for(int p=0;p<allPositions.size();p++){
                                            if (cellValue.equals(allPositions.get(p).getName())){
                                                employee.setPosid(allPositions.get(p).getId());
                                                break;
                                            }
                                        }
                                    case 15:
